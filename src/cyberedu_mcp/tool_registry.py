...
class ToolRegistry:
    ...
    def _get_parameter_info(
        ...
        elif (hasattr(param_type, '__origin__') and 
              param_type.__origin__ in (list, List)):
            param_info["type"] = "array"
            param_info["items"] = {}
        elif (hasattr(param_type, '__origin__') and 
              param_type.__origin__ in (dict, Dict)):
    ...
    def _extract_description(self, docstring: Optional[str]) -> str:
        ...
        for line in lines:
            ...
            if stripped.startswith(('Args:', 'Returns:', 'Raises:', 'Example:', 'Note:', 'Parameters:')):
                break
            if stripped:
